...
#if defined(NDEBUG)
# error "Shavercoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xee566befd24f6bf149cf47c6a50e016faa0c968175808b94c0579657dd9cc0c4");
...
const string strMessageMagic = "Shavercoin Signed Message:\n";
...
{
    // Shavercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 10 * 30; // Shavercoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Shavercoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Shavercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Shavercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xee566befd24f6bf149cf47c6a50e016faa0c968175808b94c0579657dd9cc0c4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        assert(block.hashMerkleRoot == uint256("0x28865e2c7e6d1cf1e591abe7fbfe9a182fa5cc809b98914f3dadc476ff592c2f"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // Shavercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ShavercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ShavercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ShavercoinMiner(CWallet *pwallet)
{
    printf("ShavercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("shavercoin-miner");
    ...
    try { loop {
        ...
        printf("Running ShavercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ShavercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ShavercoinMiner, pwallet));
}
